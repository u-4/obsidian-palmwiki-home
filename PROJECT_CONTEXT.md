...
- リポジトリは2026-07-12にPublicへ変更しました。最新版は[0.6.0 Release](https://github.com/u-4/obsidian-palmwiki-home/releases/tag/0.6.0)で、0.5.0、0.4.0、0.3.1、0.3.0、0.2.1、0.2.0、0.1.0も履歴として公開済みです。
...
- 0.6.1候補では、利用者によるiPhone実機受入で主要動作に大きな問題がないことを確認したため、公開manifestの`isDesktopOnly`を`false`へ変更します。実行bundleにNode.js / Electron import、`process.platform`、無条件の`FileSystemAdapter`がないことをRelease検証で固定し、iPad / Androidは継続確認項目として明記します。
...
- 0.6.0は、Vault外へ既存版をバックアップしてから`PalmWiki_LocalTest`へ3成果物だけを配置しました。Obsidian 1.12.7で検索結果・Markdown・通常Homeからの固定Homeボタン動作を確認し、Vault本文、添付ファイル、既存の設定・索引・検索キャッシュは変更していません。
- 0.6.1ではモバイルでのインストールを有効にします。iPhoneの主要動作は利用者確認済みですが、iPad miniを含む全幅とSplit View、Android、端末ごとの回転・復元、長時間利用時のメモリは継続確認項目です。その他の細目は`MANUAL_TEST_CHECKLIST.md`を正本とします。
- 0.6.0タグと公開成果物は移動・置換しません。0.6.1公開後に修正が必要な場合も既存タグは置換せず、新しい版として準備します。
