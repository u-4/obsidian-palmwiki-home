...
- リポジトリは2026-07-12にPublicへ変更しました。最新版は[0.6.1 Release](https://github.com/u-4/obsidian-palmwiki-home/releases/tag/0.6.1)で、0.6.0、0.5.0、0.4.0、0.3.1、0.3.0、0.2.1、0.2.0、0.1.0も履歴として公開済みです。
- Obsidianコミュニティプラグインへの申請はまだ行っていません。
- 本文検索は0.3.0、ItemViewからMarkdownへの互換性小修正は0.3.1、通常Markdownの検索欄とVault相対パスは0.4.0として公開済みです。
- 0.6.0はObsidian Desktop 1.12.7以上を正式対象として公開済みです。iPhone / iPadでは表示と基本操作の実機試験を開始しましたが、ライフサイクル確認が未完了のため公開manifestは引き続きdesktop-onlyです。
- 0.6.1では、利用者によるiPhone実機受入で主要動作に大きな問題がないことを確認したため、公開manifestの`isDesktopOnly`を`false`へ変更してモバイル対応版として公開しました。実行bundleにNode.js / Electron import、`process.platform`、無条件の`FileSystemAdapter`がないことをRelease検証で固定し、iPad / Androidは継続確認項目として明記します。
- 0.5.0では、横幅に追従する1行ヘッダー、更新・絞り込みアイコン、全環境で初期状態を閉じる表示設定、正方形カード、虫眼鏡付き矩形検索欄、簡素な検索候補、明るいテーマのHomeボタン・検索欄を実装しました。
- 0.6.0では、Markdown・Home検索結果・通常Homeで左上Homeボタンの行き先を固定し、旧来のページ／Obsidianコマンド選択設定を廃止しました。検索結果からはqueryだけを解除してfilter・表示方法・並べ替えを維持し、通常Homeへ戻ります。
- 正方形カードは従来の3段階サイズと独立した設定です。端末名ではなくCard領域の実幅を使い、276〜既定480 CSS pxでは2列、それより広い領域ではサイズ別に自動多列とします。2列の最大幅は設定で280〜1600 CSS pxに変更できます。パス・PageRank/In/Out・タグ・フォルダー・日付を省略し、幅変更時の仮想スクロール高さは純粋関数で再計算します。既存設定に対する形状の既定値は`portrait`です。
...
- Markdownでは通常Homeを同じタブへ表示し、Home検索結果では検索を解除、通常Homeでは最上部へ移動する固定Homeボタン動作
...
- 通常Markdownごとに所有する中央検索欄、Homeボタン直後の小さなcanonical path、検索欄設置成功時だけの中央ノートタイトル非表示（0.4.0）
- Markdown検索欄からの最近・曖昧ページ候補と、検索を開始した同じleafをPalmWiki Home本文検索へ切り替える経路（0.4.0）
...
- PR [#23](https://github.com/u-4/obsidian-palmwiki-home/pull/23)を必須CI成功後にsquash mergeし、タグ`0.6.1`は公開時の`main`コミット`b1ef401`を指しています。
- [PR CI](https://github.com/u-4/obsidian-palmwiki-home/actions/runs/29676622359)と[main CI](https://github.com/u-4/obsidian-palmwiki-home/actions/runs/29676650913)が成功しました。
- [Release workflow](https://github.com/u-4/obsidian-palmwiki-home/actions/runs/29676697460)でtagと`main`の一致、依存監査、build、モバイル向けlint、124テスト、厳格なRelease検証、3成果物の下書き作成が成功し、[PalmWiki Home 0.6.1](https://github.com/u-4/obsidian-palmwiki-home/releases/tag/0.6.1)を通常のLatest Releaseとして公開しました。
- 公開Releaseには`main.js`、`manifest.json`、`styles.css`の3点だけがあり、GitHub Digest、下書き／公開URLから再取得したファイル、ローカルbuild、`PalmWiki_LocalTest`配置物のSHA-256が一致しました。
- SHA-256は`main.js`が`34d2718dbe7336dcdd1703c79eb92305fa8b14dc5689e573ba634acf23b10399`、`manifest.json`が`87981ce37568a2da52f7721fbda8d1a25f52cba0d67ae2885c8fc158b9811de8`、`styles.css`が`26b7d509a033798f4ed848e5fe7ad55c2279b9ffa4ddb125e0f51b54f92ac330`です。

## 0.6.0公開結果

- PR [#21](https://github.com/u-4/obsidian-palmwiki-home/pull/21)を必須CI成功後にsquash mergeし、タグ`0.6.0`は公開時の`main`コミット`d725ea4`を指しています。
- [PR CI](https://github.com/u-4/obsidian-palmwiki-home/actions/runs/29629446642)と[main CI](https://github.com/u-4/obsidian-palmwiki-home/actions/runs/29629480183)が成功しました。
- [Release workflow](https://github.com/u-4/obsidian-palmwiki-home/actions/runs/29629532692)でtagと`main`の一致、依存監査、build、lint、124テスト、厳格なRelease検証、3成果物の下書き作成が成功し、[PalmWiki Home 0.6.0](https://github.com/u-4/obsidian-palmwiki-home/releases/tag/0.6.0)を通常のLatest Releaseとして公開しました。
- 公開Releaseには`main.js`、`manifest.json`、`styles.css`の3点だけがあり、GitHub Digest、下書き／公開URLから再取得したファイル、ローカルbuild、`PalmWiki_LocalTest`配置物のSHA-256が一致しました。
- SHA-256は`main.js`が`34d2718dbe7336dcdd1703c79eb92305fa8b14dc5689e573ba634acf23b10399`、`manifest.json`が`5093eea4cd22f4c2a68380f21650c3070ea3dd2276b7fc5d2e7f43f24ab50d34`、`styles.css`が`26b7d509a033798f4ed848e5fe7ad55c2279b9ffa4ddb125e0f51b54f92ac330`です。

## 0.5.0公開結果

- PR [#18](https://github.com/u-4/obsidian-palmwiki-home/pull/18)を必須CI成功後にsquash mergeし、タグ`0.5.0`は公開時の`main`コミット`fc60dc7`を指しています。
- [PR CI](https://github.com/u-4/obsidian-palmwiki-home/actions/runs/29410044094)と[main CI](https://github.com/u-4/obsidian-palmwiki-home/actions/runs/29410125218)が成功しました。
- [Release workflow](https://github.com/u-4/obsidian-palmwiki-home/actions/runs/29410359415)でtagと`main`の一致、依存監査、build、lint、132テスト、厳格なRelease検証、3成果物の下書き作成が成功し、[PalmWiki Home 0.5.0](https://github.com/u-4/obsidian-palmwiki-home/releases/tag/0.5.0)を通常のLatest Releaseとして公開しました。
- 公開Releaseには`main.js`、`manifest.json`、`styles.css`の3点だけがあり、GitHub Digest、公開URLから再取得したファイル、ローカルbuildのSHA-256が一致しました。
- SHA-256は`main.js`が`a7f87cad4f9a9e9172659a53aa3109bf272fae120b2fb83cf31feca1b3c0253a`、`manifest.json`が`d51e136a0fd26b2fec4b30c020829d7901c37aff93a3c7320a2854eb14649195`、`styles.css`が`26b7d509a033798f4ed848e5fe7ad55c2279b9ffa4ddb125e0f51b54f92ac330`です。

## 0.4.0公開結果

- PR [#16](https://github.com/u-4/obsidian-palmwiki-home/pull/16)を必須CI成功後にsquash mergeし、タグ`0.4.0`は`main`コミット`a2bb139`を指しています。
- [Release workflow](https://github.com/u-4/obsidian-palmwiki-home/actions/runs/29252133885)でtagと`main`の一致、依存監査、build、lint、118テスト、厳格なRelease検証、3成果物の下書き作成が成功し、[PalmWiki Home 0.4.0](https://github.com/u-4/obsidian-palmwiki-home/releases/tag/0.4.0)を通常のLatest Releaseとして公開しました。
- 公開Releaseには`main.js`、`manifest.json`、`styles.css`の3点だけがあり、GitHub Digest、公開URLから再取得したファイル、ローカルbuild、`PalmWiki_LocalTest`配置物のSHA-256が一致しました。
- SHA-256は`main.js`が`abcfcdf2ca77d6152f4b2e2c9bf0dd985234ea286ea193663b9d532de13ae83d`、`manifest.json`が`13b20bcd4008980fa147ea6e52e57235d503e9322e301d61dd2f233fc45cbd7a`、`styles.css`が`2cbca2f971114f3ef20f7e81066377bd9f005ea674b70499dfeac87f8bf42e4e`です。
...
- Back / Forwardでは検索語、フィルター、表示状態、スクロール位置を復元します。左上Homeボタンでは検索結果を復元せず、フィルターと表示状態を保った通常Homeへ戻ります。結果表示は100件から最大500件までとし、広すぎる検索は絞り込みを求めます。
...
- 0.6.1は、Vault外へ既存版をバックアップしてから`PalmWiki_LocalTest`へ3成果物だけを配置しました。公開manifestはモバイル対応で、Vault本文、添付ファイル、既存の設定・索引・検索キャッシュは変更していません。
- 0.6.1でモバイルインストールを有効化済みです。iPhoneの主要動作は利用者確認済みですが、iPad miniを含む全幅とSplit View、Android、端末ごとの回転・復元、長時間利用時のメモリは継続確認項目です。その他の細目は`MANUAL_TEST_CHECKLIST.md`を正本とします。
- 0.6.0／0.6.1タグと公開成果物は移動・置換しません。公開後に修正が必要な場合は0.6.2以降の新しい版として準備します。
- Windows / Linux / iPad / Androidの実機互換性は未確認であり、現時点ではmacOSとiPhoneを確認済みと記載する。
- モバイル実機検証をiPadとAndroidへ広げ、iPhoneでも縦横回転、再起動、タブ復元、バックグラウンド復帰、検索索引作成中の応答性を継続確認する。
