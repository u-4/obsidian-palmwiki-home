...
- リポジトリは2026-07-12にPublicへ変更しました。最新版は[0.5.0 Release](https://github.com/u-4/obsidian-palmwiki-home/releases/tag/0.5.0)で、0.4.0、0.3.1、0.3.0、0.2.1、0.2.0、0.1.0も履歴として公開済みです。
- Obsidianコミュニティプラグインへの申請はまだ行っていません。
- 本文検索は0.3.0、ItemViewからMarkdownへの互換性小修正は0.3.1、通常Markdownの検索欄とVault相対パスは0.4.0として公開済みです。
- 0.5.0はObsidian Desktop 1.12.7以上を正式対象として公開済みです。iPhone / iPadでは表示と基本操作の実機試験を開始しましたが、ライフサイクル確認が未完了のため公開manifestは引き続きdesktop-onlyです。
- 0.5.0では、横幅に追従する1行ヘッダー、更新・絞り込みアイコン、全環境で初期状態を閉じる表示設定、正方形カード、虫眼鏡付き矩形検索欄、簡素な検索候補、明るいテーマのHomeボタン・検索欄を実装しました。
- 0.6.0候補では、Markdown・Home検索結果・通常Homeで左上Homeボタンの行き先を固定し、旧来のページ／Obsidianコマンド選択設定を廃止しました。検索結果からはqueryだけを解除してfilter・表示方法・並べ替えを維持し、通常Homeへ戻ります。
...
- 0.5.0は、Vault外へ既存版をバックアップしてから`PalmWiki_LocalTest`へ3成果物だけを配置しました。Obsidian 1.12.7で更新状態、全表示設定、3サイズ×2形状、長いSquare一覧、明暗テーマ、最近・曖昧候補を確認しました。Vault本文と添付ファイルは変更していません。
- 0.5.0で残る主な追加スモークは、iPhone縦横回転後のライフサイクル、iPad miniを含む全幅とSplit View、ソフトウェアキーボード、再起動・タブ復元・バックグラウンド復帰、pop-out、複数splitです。現在のmanifestはdesktop-onlyのため、完了するまでiOS / Android対応とは記載しません。その他の細目は`MANUAL_TEST_CHECKLIST.md`を正本とします。
- 0.5.0タグと公開成果物は移動・置換しません。公開後に修正が必要な場合は既存版へ戻し、修正版は0.5.1以降として準備します。
- Windows / Linuxの実機互換性は未確認であり、現時点ではmacOSのみ確認済みと記載する。
- 0.5.0で始めたiOS実機検証を、縦横回転、再起動、タブ復元、バックグラウンド復帰、検索索引作成中の応答性まで広げる。macOSのiPhoneミラーリングとComputer Useは予備的な画面・タップ確認には使えるが、正式なiOS互換性の証明にはしない。
