...
- 0.6.0で残る主な追加スモークは、iPhone縦横回転後のライフサイクル、iPad miniを含む全幅とSplit View、ソフトウェアキーボード、再起動・タブ復元・バックグラウンド復帰、pop-out、複数splitです。現在のmanifestはdesktop-onlyのため、完了するまでiOS / Android対応とは記載しません。その他の細目は`MANUAL_TEST_CHECKLIST.md`を正本とします。
- 0.6.0タグと公開成果物は移動・置換しません。公開後に修正が必要な場合は既存版へ戻し、修正版は0.6.1以降として準備します。
- Windows / Linuxの実機互換性は未確認であり、現時点ではmacOSのみ確認済みと記載する。
- 0.5.0から継続するiOS実機検証を、縦横回転、再起動、タブ復元、バックグラウンド復帰、検索索引作成中の応答性まで広げる。macOSのiPhoneミラーリングとComputer Useは予備的な画面・タップ確認には使えるが、正式なiOS互換性の証明にはしない。
