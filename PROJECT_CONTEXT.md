...
- リポジトリは2026-07-12にPublicへ変更しました。最新版は[0.4.0 Release](https://github.com/u-4/obsidian-palmwiki-home/releases/tag/0.4.0)で、0.3.1、0.3.0、0.2.1、0.2.0、0.1.0も履歴として公開済みです。
- Obsidianコミュニティプラグインへの申請はまだ行っていません。
- 本文検索は0.3.0、ItemViewからMarkdownへの互換性小修正は0.3.1、通常Markdownの検索欄とVault相対パスは0.4.0として公開済みです。
- 0.5.0はObsidian Desktop 1.12.7以上を正式対象とするリリース候補です。iPhone / iPadでは表示と基本操作の実機試験を開始しましたが、ライフサイクル確認が未完了のため公開manifestは引き続きdesktop-onlyとします。
- `codex/mobile-ui-refinement`では0.5.0候補として、横幅に追従する1行ヘッダー、更新・絞り込みアイコン、全環境で初期状態を閉じる表示設定、正方形カード、虫眼鏡付き矩形検索欄、簡素な検索候補、明るいテーマのHomeボタン・検索欄を実装しました。
...
- 0.4.0の通常Markdownヘッダー検索は、Vault外へ既存版をバックアップしてから`PalmWiki_LocalTest`へ3成果物だけを配置しました。Obsidian 1.12.7でLive Preview、Source mode、Reading view、最近・曖昧候補、候補と本文検索の同一leaf遷移、Back、Focus searchコマンド、狭いsplit、plugin再読込、PalmWiki Home無効化時にも2Hop Links Plusの右上ボタンと本文内表示が維持されることを確認しました。Vault本文と添付ファイルは変更していません。
- 0.4.0で残る追加スモークは、pop-out、Hover Preview / Hover Editor / Canvas除外、splitごとの別queryとinactive leaf、rename、実際に割り当てた任意ホットキー、pointer hoverのパスツールチップ、2Hop Links Plus側だけの無効化・再有効化です。Release notesへ既知の残存確認として記載済みで、現在のmanifestはdesktop-onlyのためiOS / Androidは今回の受入対象に含めません。
- 0.4.0タグと公開成果物は移動・置換しません。公開後に修正が必要な場合は既存版へ戻し、修正版は0.4.1以降として準備します。
