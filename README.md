...
5. Homeタブ中央の検索欄へフォーカスすると、最近開いたページが表示されます。文字を入力するとページ名・ファイル名・別名の曖昧候補が表示され、矢印キーで選んでEnterを押すと同じタブで開きます。
6. 候補を選ばずに検索語をEnterで確定すると、本文検索結果へ切り替わります。空白区切りはAND、`"語句"`は連続した語句、`-除外語`は除外として扱い、本文一致、ページ名、タグ、パス、直接リンク、2-hopリンク、PageRank風スコアから順位を決めます。
7. 検索欄へ移動するコマンドは`PalmWiki Home: Focus search`です。Obsidianの「設定」→「ホットキー」で、好みのショートカットキーを割り当てられます。
