...
- Obsidian 1.12.7以上（デスクトップ／モバイル）
- 0.6.1からObsidian Mobileでのインストールと利用に対応
- macOS版Obsidian 1.12.7とiPhone実機で確認済み
- Windows / Linux / iPad / Androidは個別の実機確認が未完了

iPhone実機では、Squareカード、狭幅ヘッダー、検索欄、Home／Markdown間の移動を含む主要動作に大きな問題がないことを確認しています。端末・OS・Vault規模ごとの差があり得るため、iPadのSplit View、Android、長時間利用時のメモリなどは継続確認項目です。
...
2. 通常のMarkdownタブでは、左上のVault名ボタンの右に現在のVault相対パスを小さく表示し、中央にHomeと同じ検索欄を表示します。中央のノートタイトルはタブに表示されるため省略します。Vault名ボタンを押すと、そのMarkdownタブが通常のPalmWiki Home画面へ切り替わります。
3. PalmWiki Homeの検索結果で同じ左上ボタンを押すと検索を解除して通常のHome画面へ戻り、通常のHome画面で押すとCard / Tableのどちらでも最上部へ戻ります。
4. `Card` / `Table`で表示方法を切り替えます。設定の`Card size`は3段階、`Card shape`は情報をすべて表示する`Portrait`と、パス・タグ・リンク件数などの補助情報を省いて本文と画像を優先する`Square`から選べます。Squareは実際のカード領域が276〜既定480 CSS pxなら2列を保ち、それより広ければCard sizeに応じて列数を増やします。276未満では安全のため1列です。2列の境界は`Square two-column maximum width`で280〜1600 CSS pxに変更できます。CardまたはTableのページ名をクリックすると、そのHomeタブが選択したMarkdownページへ置き換わり、新しいタブは増えません。`Card preview`設定では、カードのページプレビューを無効、Cmd/Ctrl＋hover、hoverのみから選べます。
...
9. 並べ替え、昇順・降順、フォルダー、タグ、Quick filter、`Links to page`を組み合わせて絞り込みます。これらのフィルターは本文検索結果にも適用されます。Home上部では、利用できる横幅が十分ならタイトルの右側に状態表示・更新・絞り込みアイコンを1行で並べます。絞り込みアイコンから、初期状態で閉じた表示設定を開きます。狭い画面や分割ペインでは状態行だけを次の行へ送ります。
10. `Pin`は通常一覧では先頭グループに固定します。本文検索では関連度を上書きせず、同じ関連度の結果だけを優先します。
11. 最新状態をすぐ確認したい場合は、索引状態の隣にある更新アイコンを選びます。
...
- `Update failed`: 更新に失敗しました。保存済み一覧があれば引き続き表示し、索引状態の隣にある更新アイコンで再試行できます。
...
- Markdown・Home検索結果・通常Homeで迷わず同じHomeへ戻れる固定Homeボタン動作
- Card / Tableからクリック元のHomeタブへMarkdownページを開く同一タブ遷移
- CardからObsidian標準のページプレビューを開く3段階設定
- 3段階のカードサイズと、安全幅では2列・広い幅では自動多列・極端に狭い幅では1列になる正方形カード（2列境界を設定可能）
...
1. Home上部で、索引状態の隣にある更新アイコンを選びます。
