...
前回の索引は、各Vaultの`.obsidian/plugins/palmwiki-home/index-cache.json`へ保存します。保存対象はタイトル、Vault内パス、タグ、リンク情報、行数・文字数、および各ノート本文から作る最大180文字の説明などの表示用メタデータで、ノート全文は保存しません。このファイルはVault内だけで使う実行時キャッシュであり、Git管理やGitHub Releaseには含めません。Vault設定フォルダーをObsidian Sync等の対象にしている場合は、その仕組みによって同期される可能性があります。破損、旧形式、索引設定の変更時は安全に無視して再構築します。
