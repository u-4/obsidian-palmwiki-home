...
- 0.1.0はデスクトップ版のみ対応
- macOS版Obsidian 1.12.7で確認
- iOS / Android対応は0.1.0以降に検証予定
...
2. `Card` / `Table`で表示方法を切り替えます。
3. 並べ替え、昇順・降順、フォルダー、タグ、Quick filter、`Links to page`を組み合わせて絞り込みます。
4. `Pin`で重要なページを先頭グループに固定します。
5. 最新状態をすぐ確認したい場合は`Refresh`を選びます。
...
- `data.json`: include / exclude、表示方法、ピン留めなどの設定
