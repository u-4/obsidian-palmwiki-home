...
### 0.5.0 responsive UI candidate

- At a Home content width above 620 CSS pixels, confirm `PalmWiki Home` remains left aligned and both compact index-state badges, the standard refresh icon, and the filter icon form one right-aligned row. At 620 and below, confirm only the status row wraps below the title. Long visible labels may end in an ellipsis, but the complete status remains in the accessible label. Tap refresh once and confirm one display-index refresh and one search-index refresh start; no large text Refresh row appears.
- Confirm display settings are closed whenever a PalmWiki Home view is first created on desktop, iPhone, and iPad. Confirm the icon-only filter button remains identifiable through its accessibility label. Open it, use Card/Table, sort direction, folder, tag, Quick filter, and Links to page, then close and reopen it. Confirm controls remain usable, selected filters add the small indicator dot, typing an unselected Links to page candidate does not add the dot, and closing it removes any link-target popup.
- In plugin settings, select each of Small/Medium/Large with Portrait and Square. Confirm Portrait keeps its historical responsive columns. With the recommended Square breakpoint of 480, confirm card-area widths from 320 through 440 stay at exactly two columns for every Card size, 480 is inclusive, and wider iPad/desktop layouts adapt to Card size without dropping below two columns. Confirm widths below 276 safely use one column.
- Change `Square two-column maximum width` to valid values from 280 through 1600 and confirm the boundary moves immediately without an index rebuild. Use its reset button and confirm 480 is restored. Rotate an iPhone/iPad and resize an iPad Split View or desktop split across the boundary; confirm no duplicate cards, row overlap, large empty gaps, or unreachable final row appears.
- Check Square cards with image/no image, description/no description, long titles, and pinned/unpinned state. Confirm title, image, description, and compact Pin remain; path fallback, graph badges, tags, folder/path, and date are absent. Switch back to Portrait and confirm all configured metadata returns.
- In light theme, confirm the upper-left Home button and search field use a white surface on Home and normal Markdown leaves. Confirm the search field is an almost-square rectangle with a slight corner radius, has a leading magnifying-glass icon, and has no visible `Search pages` placeholder. Repeat in dark theme and confirm contrast remains readable. Confirm the custom clear control does not overlap a native iOS search decoration.
- Focus the empty Home and Markdown search fields. Confirm compact flat recent rows appear with title as the primary line and the complete canonical path, smaller and right-aligned below it. In light theme, confirm the popup itself is white and text contrast remains readable. Repeat with alias/fuzzy input, keyboard selection, touch selection, scrolling, and no-result state.
